...
class ToggleSwitchState extends State<ToggleSwitch> with AutomaticKeepAliveClientMixin<ToggleSwitch> {
  ...
  Widget build(BuildContext context) {
    ...
    final int totalSwitches = widget.totalSwitches ?? [widget.labels?.length ?? 0, widget.icons?.length ?? 0, widget.customIcons?.length ?? 0].reduce(max);
    super.build(context);
    ...
    return ClipRRect(
      ...
      child: Container(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: inactiveBgColor, borderRadius: widget.radiusStyle ? widget.customRadius : BorderRadius.circular(widget.cornerRadius)),
          ...
          child: RowToColumn(
            ...
            children: List.generate(totalSwitches * 2 - 1, (index) {
              ...
              if (active) {
                bgColor = widget.activeBgColors == null ? activeBgColor : (widget.activeBgColors![index ~/ 2] ?? activeBgColor);
              }

              if (index % 2 == 1) {
                final activeDivider = active || (widget.initialLabelIndex != null && index ~/ 2 == widget.initialLabelIndex! - 1);
                ...
                return Container(
                  ...
                  margin: widget.isVertical ? EdgeInsets.symmetric(horizontal: activeDivider ? 0.0 : widget.dividerMargin!) : EdgeInsets.symmetric(vertical: activeDivider ? 0.0 : widget.dividerMargin!),
                );
              } else {
                ...
                if (widget.customTextStyles != null) {
                  textStyle = widget.customTextStyles!.length == 1 ? widget.customTextStyles![0]! : (widget.customTextStyles!.length > index ~/ 2 && widget.customTextStyles![index ~/ 2] != null ? widget.customTextStyles![index ~/ 2]! : defaultTextStyle);
                }
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: widget.radiusStyle ? widget.customRadius : cornerRadius,
                      gradient: LinearGradient(
  ...
  double _calculateWidth(int index, int totalSwitches) {
    ...
    return widget.customWidths != null ? widget.customWidths![index] : ((totalSwitches + extraWidth) * widget.minWidth < screenWidth ? widget.minWidth : screenWidth / (totalSwitches + extraWidth));
  }
  ...
  double _calculateHeight(int index, int totalSwitches) {
    ...
    return widget.customHeights != null ? widget.customHeights![index] : ((totalSwitches + extraHeight) * widget.minHeight < screenHeight ? widget.minHeight : screenHeight / (totalSwitches + extraHeight));
  }
}
